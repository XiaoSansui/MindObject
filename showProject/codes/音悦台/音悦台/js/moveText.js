!(function (w) {
  ...
  w.moveText.vMove = function (wrap,callBack) {

    var item = wrap.children[0];
    moveText.css(item,"translateZ",0.1);
    ...
    wrap.addEventListener("touchstart",function (ev) {
      ...
      var mouse = {clientX:touchC.clientX,clientY:touchC.clientY};
      element.y = moveText.css(item,"translateY");
      element.x = moveText.css(item,"translateX");
    ...
    wrap.addEventListener("touchmove",function (ev) {
      ...
      var touchC = ev.changedTouches[0];
      /*var nowY = touchC.clientY;
      var dirY = nowY - mouseY;
      var translateY = elementY+dirY;*/
      var now =touchC;
      var dis ={};
      dis.x =now.clientX - mouse.clientX;
      dis.y =now.clientY - mouse.clientY;
   ...
   * */
      if(translateY>0){
        ...
        translateY = moveText.css(item,"translateY")+pointDir*scale;
      ...
      moveText.css(item,"translateY",translateY);
    ...
    wrap.addEventListener("touchend",function (ev) {
      var translateY = moveText.css(item,"translateY")
      if(!item.handMove){
        //快速滑屏
        //速度越大  位移越远
        ...
        var speed = pointDir/timeDir;
        speed = Math.abs(speed)<0.5?0:speed;
        var speedY = translateY+ speed*200;
        //var bsr = "";
        var time= Math.abs(speed)*0.2;
    ...
    function bsr(type,speedY,time){
      clearInterval(cleartime);
      //当前次数
      var t=0;
      //初始位置
      var b = moveText.css(item,"translateY");
      //最终位置 - 初始位置
      var c = speedY -b;
      //总次数
      ...
      cleartime = setInterval(function(){
        ...
        moveText.css(item,"translateY",point);
      },1000/60);
    }
  }
})(window)