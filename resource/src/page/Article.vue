<template>
  <div>
    ...
    <div class="cardWrap clearfix">
      <div class="card" v-for="article in articleList" :key="article.id"  @click="detailId(article.id,article.siteType)">
        <router-link to="/details">
          <div class="left" :style="imgUrl(article.thumbnailImg)">
          </div>
          <div class="right">
            <p><span class="rightText">&nbsp;&nbsp;{{article.title}}</span></p>
            <p class="rightContent">
              {{article.summary}}
            </p>
            <div class="rightBottom">
              <div class="author">
                <i class="el-icon-edit-outline"></i>&nbsp;<span>{{article.author}}</span>
              </div>
              <div class="read">
                <i class="el-icon-view"></i>&nbsp;<span>{{article.clickCount}}</span>
              </div>
              <div class="time">
                <i class="el-icon-time"></i>&nbsp;<span>{{ article.addTime | dateformat('YYYY-MM-DD HH:mm:ss')}}</span>
              </div>
            </div>
          </div>
        </router-link>
      </div>
      <div class="scene" v-show="!articleList" style="width: 30%;margin: 0 auto">
        <svg
          version="1.1"
          id="dc-spinner"
        ...
        if(this.articleItems){
          switch (value[0]) {
            case 'quanbu':
              ...
              break;
            case 'kaifayuyan':
              this.articleList = this.articleItems.filter((item,x)=>{
                return this.articleItems[x].catalogId===1
              })
              break;
            case 'meishi':
              this.articleList = this.articleItems.filter((item,x)=>{
                return this.articleItems[x].catalogId===13
              })
              break;
            case 'huazhuang':
              this.articleList = this.articleItems.filter((item,x)=>{
                return this.articleItems[x].catalogId===14
              })
              break;
            case 'shishang':
              this.articleList = this.articleItems.filter((item,x)=>{
                return this.articleItems[x].catalogId===15
              })
              break;
          }
          switch (value[1]) {
            ...
            case 'java':
              this.articleList = this.articleItems.filter((item,x)=>{
                return this.articleItems[x].catalogId===2
              })
              break;
            case 'php':
              this.articleList = this.articleItems.filter((item,x)=>{
                return this.articleItems[x].catalogId===28
              })
              break;
          }
        }
      ...
      selectedOptions2(value){
        switch (value[0]) {
          case 'moren':
            ...
            break;
          case 'zuixin':
            this.articleList = this.articleItems.sort((a,b)=>{
